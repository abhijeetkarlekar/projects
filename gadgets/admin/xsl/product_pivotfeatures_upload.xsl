...
<title>Admin Upload Product Features</title>
...
<script LANGUAGE="JavaScript" SRC="{XML/ADMIN_JS_URL}product_pivotfeatures_upload.js"></script>
	...
	<div class="container_12">
		...
		<div class="grid_10">
			<div class="box round first grid">
				<h2>Upload Pivot Features Parameters Data</h2>
				<div class="block">
					<table class="form">
						...
						<tr>
							<td>
								...
								<div>
									<button class="btn btn-navy" name="select_category" id="select_category" onclick="javascript: getTopSellingCarDashboard('prod_article_dashboard','productajaxloader','','{/XML/STARTLIMIT}','{/XML/CNT}');">Select Category</button>
								</div>
							</td>
						</tr>
					</table>
		...
		<div id="prod_article_dashboard" style="display:block;">
                ...
                Please select category for Pivot Features Parameters information.
...
$(document).ready(function() {
                ...
                <xsl:if test="/XML/SELECTED_CATEGORY_ID!=''">
                        getTopSellingCarDashboard('prod_article_dashboard','productajaxloader','<xsl:value-of select="/XML/SELECTED_CATEGORY_ID" diseable-output-escaping="yes"/>','<xsl:value-of select="/XML/STARTLIMIT" diseable-output-escaping="yes"/>','<xsl:value-of select="/XML/CNT" diseable-output-escaping="yes"/>');
                </xsl:if>
            });
