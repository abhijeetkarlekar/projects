...
<title> Feature Overview Admin</title>
...
<script LANGUAGE="JavaScript" SRC="{XML/ADMIN_JS_URL}feature_overview.js"></script>
...
<body>
	<div class="container_12">
					...
					<table class="form">
						...
						<tr>
							<td>
								...
								<div>
									<button class="btn btn-navy" name="select_category" id="select_category" onclick="javascript:getFeatureOverviewDashboard('feature_overview_dashboard','featureoverviewajaxloader','','{/XML/STARTLIMIT}','{/XML/CNT}');">Select Category</button>
								</div>
							</td>
						</tr>
					</table>
		...
		<div id="feature_overview_dashboard" style="display:block;">Please select category for Feature Overview information.
...
var group_id = '<xsl:value-of select="/XML/SELECTED_MAIN_GROUP_ID" disable-output-escaping="yes"/>';
                ...
                <xsl:if test="/XML/SELECTED_CATEGORY_ID!=''">
                    getFeatureOverviewDashboard('feature_overview_dashboard','featureoverviewajaxloader','<xsl:value-of select="/XML/SELECTED_CATEGORY_ID" diseable-output-escaping="yes"/>','<xsl:value-of select="/XML/STARTLIMIT" diseable-output-escaping="yes"/>','<xsl:value-of select="/XML/CNT" diseable-output-escaping="yes"/>');
                </xsl:if>
                });
                 setupLeftMenu();
        $('.datatable').dataTable();
