...
<title>Add Best Seller Product</title>
...
<script LANGUAGE="JavaScript" SRC="{XML/ADMIN_JS_URL}best_seller_product.js"></script>
	...
	<div class="container_12">
		...
		<div class="grid_10">
			<div class="box round first grid">
				<h2>Best Seller Product Managemnt</h2>
				<div class="block">
					<table class="form">
						...
						<tr>
							<td>
								...
								<div>
									<button class="btn btn-navy" name="select_category" id="select_category" onclick="javascript: getProductArticleDashboard('prod_article_dashboard','productajaxloader','','{/XML/STARTLIMIT}','{/XML/CNT}');">Select Category</button>
								</div>
							</td>
						</tr>
					</table>
		...
		<div id="prod_article_dashboard" style="display:block;">
                ...
                Please select category for Best Seller Product information.
...
$(document).ready(function() {
                ...
                <xsl:if test="/XML/SELECTED_CATEGORY_ID!=''">
                        getProductArticleDashboard('prod_article_dashboard','productajaxloader','<xsl:value-of select="/XML/SELECTED_CATEGORY_ID" diseable-output-escaping="yes"/>','<xsl:value-of select="/XML/STARTLIMIT" diseable-output-escaping="yes"/>','<xsl:value-of select="/XML/CNT" diseable-output-escaping="yes"/>');
                </xsl:if>
            });
