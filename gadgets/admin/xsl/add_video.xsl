...
<script language="javascript" src="{XML/ADMIN_JS_URL}video.js"></script>
...
function addRemoveFileBrowseElements(iAddOrRemove){
	...
	if(iAddOrRemove==0){
		...
		oTdUploadMedia.innerHTML="<label>Video "+iTotalRowsCurrent+":</label>";
						...
						<tr>
							<td>
								...
								<div >
                                            ...
                                            <button class="btn btn-navy" name="select_category" id="select_category" onclick="javascript: getVideosDashboard('prod_article_dashboard','productajaxloader','','{/XML/STARTLIMIT}','{/XML/CNT}');">Select Category</button>	
                                        </div>
							</td>
						</tr>
					</table>
			...
			var siteURL = '<xsl:value-of select="/XML/ADMIN_WEB_URL" disable-output-escaping="yes"/>';
             ...
             $(document).ready(function() {
                ...
                <xsl:if test="/XML/SELECTED_CATEGORY_ID!=''">
                        getVideosDashboard('prod_article_dashboard','productajaxloader','<xsl:value-of select="/XML/SELECTED_CATEGORY_ID" diseable-output-escaping="yes"/>','<xsl:value-of select="/XML/STARTLIMIT" diseable-output-escaping="yes"/>','<xsl:value-of select="/XML/CNT" diseable-output-escaping="yes"/>');
                </xsl:if>
            });
