...
<title>Add Pivot Management</title>
...
<script LANGUAGE="JavaScript" SRC="{XML/ADMIN_JS_URL}pivot.js"></script>
...
<body>
	<div class="container_12">
					...
					<table class="form">
						...
						<tr>
							<td>
								...
								<div>
									<button class="btn btn-navy" name="select_category" id="select_category" onclick="javascript:getPivotDashboard('pivot_dashboard','pivotajaxloader','','{/XML/STARTLIMIT}','{/XML/CNT}');">Select Category</button>
								</div>
							</td>
						</tr>
					</table>
		...
		<div id="pivot_dashboard" style="display:block;">
			
    		
                Please select category for pivot information.
    		
		</div>
		<div class="clear">
		</div>
	</div>
	<div id="pivotajaxloader" style="display:none;">
        <div align="center">
            <img src="{/XML/ADMIN_IMAGE_URL}ajax-loader.gif"/>
        </div>
    </div>
...
$(document).ready(function() {
                ...
                <xsl:if test="/XML/SELECTED_CATEGORY_ID!=''">
                        getPivotDashboard('pivot_dashboard','pivotajaxloader','<xsl:value-of select="/XML/SELECTED_CATEGORY_ID" diseable-output-escaping="yes"/>','<xsl:value-of select="/XML/STARTLIMIT" diseable-output-escaping="yes"/>','<xsl:value-of select="/XML/CNT" diseable-output-escaping="yes"/>');
                </xsl:if>
            });
</script>
</html>
