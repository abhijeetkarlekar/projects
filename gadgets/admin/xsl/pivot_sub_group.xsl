...
<script LANGUAGE="JavaScript" SRC="{XML/ADMIN_JS_URL}pivot_sub_group.js"></script>
...
<body>
	<div class="container_12">
					...
					<table class="form">
						...
						<tr>
							<td>
								...
								<div>
									<button class="btn btn-navy" name="select_category" id="select_category" onclick="javascript:getPivotGroupDashboard('pivot_group_dashboard','pivotgroupajaxloader','','{/XML/STARTLIMIT}','{/XML/CNT}');">Select Category</button>
								</div>
							</td>
						</tr>
					</table>
		...
		<div id="pivot_group_dashboard" style="display:block;">
	...
	<div id="pivotgroupajaxloader" style="display:none;">
        <div align="center">
            <img src="{/XML/ADMIN_IMAGE_URL}ajax-loader.gif"/>
        </div>
    </div>
...
$(document).ready(function() {
                ...
                <xsl:if test="/XML/SELECTED_CATEGORY_ID!=''">
                    getPivotGroupDashboard('pivot_group_dashboard','pivotgroupajaxloader','<xsl:value-of select="/XML/SELECTED_CATEGORY_ID" diseable-output-escaping="yes"/>','<xsl:value-of select="/XML/STARTLIMIT" diseable-output-escaping="yes"/>','<xsl:value-of select="/XML/CNT" diseable-output-escaping="yes"/>');
                </xsl:if>
                });
</script>
</html>
