<?php
	...
	if($actiontype == 'Delete'){
	   $result = $overview->boolDeleteCarFinderFeaturedOverview($overview_id);
	   $msg = 'Feature Overview deleted successfully.';
	}elseif($actiontype == 'Update' && !empty($feature_id)){
	   //$result = $brand->boolUpdateCarFinderFeaturedOverview($overview_id,$request_param);
	  // $msg = 'Brand updated successfully.';
	}elseif($actiontype == 'insert' && !empty($feature_id)){
	   $result = $overview->intInsertCarFinderFeaturedOverview($request_param);	 
	...
	$xsl = DOMDocument::load('xsl/car_finder_feature_overview.xsl');
