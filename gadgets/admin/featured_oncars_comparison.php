<?php
	...
	if($actiontype == 'Insert'|| $actiontype== 'Update'){
		unset($request_param);
		$featured_compare_id = $_REQUEST["hd_featured_compare_id"];
		...
		if($select_section_id == "FEATURED_COMPARISON_SET"){
                	$table_name = "FEATURED_ONCARS_COMPARISON";
		}
		
	    if($actiontype == 'Insert'){
		       $result = $product->addUpdFeaturedCompareSetDetails($request_param,$table_name);
        	       if($sresult>0){$msg = 'Featured compare set added successfully.';}
		}elseif($actiontype == 'Update'){
			$request_param['featured_compare_id'] = $featured_compare_id;
			$result = $product->addUpdFeaturedCompareSetDetails($request_param,$table_name);
        		if($sresult>0){$msg = 'Featured compare set updated successfully.';}
		}
	...
	if($actiontype == 'Delete'){
		$featured_compare_id = $_REQUEST["hd_featured_compare_id"];
		$hd_view_section_id = $_REQUEST["hd_view_section_id"];
		if($hd_view_section_id == "FEATURED_COMPARISON_SET"){
                        $table_name = "FEATURED_ONCARS_COMPARISON";
                }
        	if($featured_compare_id!=''){
                	$result = $product->boolDeleteFeaturedCompareSetDetail($featured_compare_id,$table_name);
	...
	if($_GET['debug']==1) { header('Content-type: text/xml');echo $strXML;exit; }
        ...
        $xsl = DOMDocument::load('xsl/featured_oncars_comparison.xsl');
