<?php
	...
	if($_POST['product_id']!='' && strlen($_POST['product_id'])>0){
		...
		$featured_product_id==0 ? $aParameters['create_date']=date("Y-m-d H:i:s") : $aParameters['update_date']=date("Y-m-d H:i:s"); 
               # print"<pre>";print_r($aParameters);exit;
		if($featured_product_id>0) $aParameters['featured_product_id']=$featured_product_id;
		$iResId=$oProduct->intInsertFeaturedProduct($aParameters);
	 	$featured_product_id=$iResId;
	...
	if($actiontype == 'Delete'){
	   $result = $oProduct->boolDeleteFeaturedProduct($featured_product_id);
...
$xsl = DOMDocument::load('xsl/featured_product.xsl');
