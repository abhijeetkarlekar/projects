<?php
	...
	$arrival_product_id=$_REQUEST['arrival_product_id'];
	if($_POST['product_id']!='' && strlen($_POST['product_id'])>0){
		...
		$arrival_product_id==0 ? $aParameters['create_date']=date("Y-m-d H:i:s") : $aParameters['update_date']=date("Y-m-d H:i:s"); 
               # print"<pre>";print_r($aParameters);exit;
		if($arrival_product_id>0) $aParameters['arrival_product_id']=$arrival_product_id;
		$iResId=$oProduct->intInsertNewArrivalProduct($aParameters);
	 	$arrival_product_id=$iResId;
		
		if($arrival_product_id==0) {
			$arrival_product_id=$iResId;
	...
	if($actiontype == 'Delete'){
	   $result = $oProduct->boolDeleteNewArrivalProduct($arrival_product_id);
...
$xsl = DOMDocument::load('xsl/newarrival_product.xsl');
