<?php
	...
	if($_POST['product_id']!='' && strlen($_POST['product_id'])>0){
		...
		$top_product_id==0 ? $aParameters['create_date']=date("Y-m-d H:i:s") : $aParameters['update_date']=date("Y-m-d H:i:s"); 
               # print"<pre>";print_r($aParameters);exit;
		if($top_product_id>0) $aParameters['top_product_id']=$top_product_id;
		$iResId=$oProduct->intInsertTopProduct($aParameters);
	 	$top_product_id=$iResId;
	...
	if($actiontype == 'Delete'){
	   $result = $oProduct->boolDeleteTopProduct($top_product_id);
...
$xsl = DOMDocument::load('xsl/top_product.xsl');
