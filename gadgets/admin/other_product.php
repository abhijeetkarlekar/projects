<?php
	...
	if($_POST['product_id']!='' && strlen($_POST['product_id'])>0){
		...
		$other_product_id==0 ? $aParameters['create_date']=date("Y-m-d H:i:s") : $aParameters['update_date']=date("Y-m-d H:i:s"); 
               # print"<pre>";print_r($aParameters);exit;
		if($other_product_id>0) $aParameters['other_product_id']=$other_product_id;
		$iResId=$oProduct->intInsertOtherProduct($aParameters);
	 	$other_product_id=$iResId;
		
		if($other_product_id==0) {
			$other_product_id=$iResId;
	...
	if($actiontype == 'Delete'){
	   $result = $oProduct->boolDeleteOtherProduct($other_product_id);
...
$xsl = DOMDocument::load('xsl/other_product.xsl');
