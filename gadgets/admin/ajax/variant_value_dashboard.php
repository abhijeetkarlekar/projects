<?php
	...
	if($_REQUEST['act']=='update' && !empty($price_variant)){

		$category_id = $category_id ? $category_id : $_REQUEST['catid'];
		$aParameters=array('category_id'=>$category_id);
		$rResult = $oPrice->arrGetVariantValueDetail($price_variant,"","",$category_id,"","","","","","","");
		$xmlArt='';
		$cnt = sizeof($rResult);
		...
		$variant_id = $rResult[0]['variant_id'];
		...
		$rResult[0]['update_date'] = date('d-m-Y',strtotime($rResult[0]['udate']));
...
//die();
	...
	if(!empty($category_id)){
		$aParameters=array('category_id'=>$category_id);
		$result = $oPrice->arrGetVariantValueDetail("","","",$category_id,"","","","",0,100);
	}
	*/
	...
	$xsl = DOMDocument::load('../xsl/variant_value_dashboard.xsl');
