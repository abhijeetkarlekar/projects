<?php
	...
	unset($result);
	if(!empty($category_id)){
		$GetFeatureOverviewcnt = Array();
		$GetFeatureOverviewcnt = $overview->arrGetCarFinderFeatureOverview("","",$category_id,"","","","","","","");
		...
		if($iGetFeatureviewCount!=0){
			...
			$sExtraParam="ajax/car_finder_feature_overview_ajax.php,sFeatureOverDiv,$category_id";
			$jsparams=$start.",".$perpage.",".$sExtraParam;
			$pages= $oPager->findPages($recordcount,$perpage);
			if($pages > 1 ){
				$pagelist= $oPager->jsPageNumNextPrev($page,$pages,"sCarFinderFeatureOverPagination",$jsparams,"text");
			 ...
			 $result = $overview->arrGetCarFinderFeatureOverview("","",$category_id,"","","","","",$start,$perpage);
	 	}
	}
	$cnt = sizeof($result);
	...
	for($i=0;$i<$cnt;$i++){
		...
		$result[$i]['feature_name'] = $feature_name ? html_entity_decode($feature_name,ENT_QUOTES) : 'Nil';;
		...
		$result[$i]['abbreviation'] = $abbrivation ? html_entity_decode($abbrivation,ENT_QUOTES) : 'Nil';
		...
		$result[$i]['title'] = $title ? html_entity_decode($title,ENT_QUOTES) : 'Nil';
	...
	$xsl = DOMDocument::load('../xsl/car_finder_feature_overview_ajax.xsl');
