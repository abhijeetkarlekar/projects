<?php	
	...
	if(!empty($category_id)){
		...
		$result = $product->arrGetTopProductDetails("","",$category_id,"","",$startlimit,$limitcnt);
	}

	if($_REQUEST['top_product_id']!=''){
		$top_product_id = $_REQUEST['top_product_id'];
	}
	if($_REQUEST['act']=='Delete' && !empty($top_product_id)){
			$dresult = $product->boolDeleteTopProduct($top_product_id);

	}
	
	if($_REQUEST['act']=='update' && !empty($top_product_id)){
                
		$rResult =$product->arrGetTopProductDetails($top_product_id,"","","","",$startlimit,$limitcnt);
...
//print_r($result);
	...
	$xsl = DOMDocument::load('../xsl/top_product_dashboard.xsl');
