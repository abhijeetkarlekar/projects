<?php	
	...
	$latest_product_id = $_REQUEST['lpid'];
	...
	if(!empty($category_id)){
		...
		$result = $product->arrGetProductLatestDetails("","",$category_id,"","",$startlimit,$limitcnt);
	}

	if($_REQUEST['latest_product_id']!=''){
		$latest_product_id = $_REQUEST['latest_product_id'];
	}
	if($_REQUEST['act']=='Delete' && !empty($latest_product_id)){
			$dresult = $product->boolDeleteLatestProduct($latest_product_id);

	}
	
	if($_REQUEST['act']=='update' && !empty($latest_product_id)){
                
		$rResult =$product->arrGetProductLatestDetails($latest_product_id,"","","","",$startlimit,$limitcnt);
...
//print_r($result);
	...
	$xsl = DOMDocument::load('../xsl/latest_product_dashboard_ajax.xsl');
