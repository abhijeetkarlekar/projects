<?php	
	...
	$arrival_product_id = $_REQUEST['lpid'];
	...
	if($_REQUEST['arrival_product_id']!=''){
		$arrival_product_id = $_REQUEST['arrival_product_id'];
	}
	if($_REQUEST['act']=='Delete' && !empty($arrival_product_id)){
			$dresult = $product->boolDeleteNewArrivalProduct($arrival_product_id);

	}
	
	if($_REQUEST['act']=='update' && !empty($arrival_product_id)){
                
		$rResult =$product->arrGetNewArrivalProductDetails($arrival_product_id,"","","","",$startlimit,$limitcnt);
...
//print_r($result);
	...
	$xsl = DOMDocument::load('../xsl/newarrival_product_dashboard.xsl');
