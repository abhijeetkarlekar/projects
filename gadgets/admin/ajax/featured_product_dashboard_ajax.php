<?php	
	...
	$featured_product_id = $_REQUEST['lpid'];
	...
	if($_REQUEST['featured_product_id']!=''){
		$featured_product_id = $_REQUEST['featured_product_id'];
	}
	if($_REQUEST['act']=='Delete' && !empty($featured_product_id)){
			$dresult = $product->boolDeleteFeaturedProduct($featured_product_id);

	}
	
	if($_REQUEST['act']=='update' && !empty($featured_product_id)){
                
		$rResult =$product->arrGetProductFeaturedDetails($featured_product_id,"","","","",$startlimit,$limitcnt);
...
//print_r($result);
	...
	$xsl = DOMDocument::load('../xsl/featured_product_dashboard_ajax.xsl');
