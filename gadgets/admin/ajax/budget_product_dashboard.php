<?php	
	...
	$budget_product_id = $_REQUEST['lpid'];
	...
	if($_REQUEST['budget_product_id']!=''){
		$budget_product_id = $_REQUEST['budget_product_id'];
	}
	if($_REQUEST['act']=='Delete' && !empty($budget_product_id)){
			$dresult = $product->boolDeleteBudgetProduct($budget_product_id);

	}
	
	if($_REQUEST['act']=='update' && !empty($budget_product_id)){
                
		$rResult =$product->arrGetBudgetProductDetails($budget_product_id,"","","","",$startlimit,$limitcnt);
...
//print_r($result);
	...
	$xsl = DOMDocument::load('../xsl/budget_product_dashboard.xsl');
