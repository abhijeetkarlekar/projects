<?php
	...
	if($_POST['product_id']!='' && strlen($_POST['product_id'])>0){
			...
			$trending_product_id==0 ? $aParameters['create_date']=date("Y-m-d H:i:s") : $aParameters['update_date']=date("Y-m-d H:i:s"); 
			// print"<pre>";print_r($aParameters);exit;
			if($trending_product_id>0) $aParameters['trending_product_id']=$trending_product_id;
			$iResId=$oProduct->intInsertTrendingProduct($aParameters);
			$trending_product_id=$iResId;
		
		if($trending_product_id==0) {
			$trending_product_id=$iResId;
	...
	if($actiontype == 'Delete'){
	   $result = $oProduct->boolDeleteTrendingProduct($trending_product_id);
...
$xsl = DOMDocument::load('xsl/trending_product.xsl');
