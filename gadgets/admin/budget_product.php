<?php
	...
	$budget_product_id=$_REQUEST['budget_product_id'];
	if($_POST['product_id']!='' && strlen($_POST['product_id'])>0){
		...
		$budget_product_id==0 ? $aParameters['create_date']=date("Y-m-d H:i:s") : $aParameters['update_date']=date("Y-m-d H:i:s"); 
               # print"<pre>";print_r($aParameters);exit;
		if($budget_product_id>0) $aParameters['budget_product_id']=$budget_product_id;
		$iResId=$oProduct->intInsertBudgetProduct($aParameters);
	 	$budget_product_id=$iResId;
		
		if($budget_product_id==0) {
			$budget_product_id=$iResId;
	...
	if($actiontype == 'Delete'){
	   $result = $oProduct->boolDeleteBudgetProduct($budget_product_id);
...
$xsl = DOMDocument::load('xsl/budget_product.xsl');
