...
//        die();
        if (!empty($error_arr)) {
            ...
            ob_clean();
            flush();
            file_put_contents("php://output", $error_arr);
            exit;
        } else {
            //echo"No error found"; die;
            $rcnt = count($rowData);
            ...
            $featurecount = (int) $pivotfeaturecnt;
            for ($i = 5; $i <= $rcnt; $i++) {
                $brand_name = trim($rowData[$i][0][0]);
                ...
                if (sizeof($brandres) <= 0) {
                    ...
                    $brand_id = $brandres[0]['brand_id'];
                }
                $model_name = trim($rowData[$i][0][1]);
                 ...
                 */
                  $variant_name=trim($rowData[$i][0][2]); 
                ...
                if (!empty($model_name)) {
                    $model_product_result = $oProduct->arrGetProductDetails("", "", "", '1', "", "", "1", "", "", "1", "", $model_name, "", "", '', "", '0', $variant_name);
                    echo "COUNT--" . $featurecount . "<br>"; //die;
                    //echo "<pre>"; print_r($model_product_result);
